    ...
    {stats->Belt.Array.length > 0
      ? <div className="flex gap-4">
          {Belt.Array.mapWithIndex(stats, (index, (value, label)) => {
            <div className="flex flex-col" id={`ProjectStatsCard_Item_${Belt.Int.toString(index)}`}>
              <span className="text-lg font-semibold text-purple-500"> {React.string(value)} </span>
